...
class TeleopObservation:
    ...
    def __init__(
        ...
        rgb_images: Optional[Dict[str, NDArray[np.float64]]] = None,  # camera images
        depth_images: Optional[Dict[str, NDArray[np.float64]]] = None,  # depth images
    ):
...
class TeleopWrapper(Device[TeleopCommand]):
    ...
    def __init__(self, movement_speed: float = 0.01, rotation_speed: float = 0.05):
        super().__init__()
        ...
        self.current_position: Optional[NDArray[np.float64]] = None  # Will be set by robot's initial pose
        self.current_orientation: Optional[NDArray[np.float64]] = None  # Will be set by robot's initial pose
    ...
    def _on_press(self, key: Union[keyboard.Key, keyboard.KeyCode, None]) -> None:
        """Handle key press events."""
        with self.lock:
            self.pressed_keys.add(key)
    
    def _on_release(self, key: Union[keyboard.Key, keyboard.KeyCode, None]) -> None:
